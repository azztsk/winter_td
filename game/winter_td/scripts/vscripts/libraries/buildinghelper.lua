...
local flood4stack = require("libraries/floodfill.flood4stack")
local flood8stack = require("libraries/floodfill.flood8stack")
local Stack = require("libraries/struct.stack")
local Grid = require ('libraries/struct.grid')
...
function BuildingHelper:Init()
	...
	-- Stores the location of each bh_blocked position - required by the anti-blocking algorithm
	BuildingHelper.bh_blocked_positions = {}
...
function BuildingHelper:OnGameRulesStateChange(keys)
    ...
    if newState == DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP then
		...
		local endPointVectorPosition = endPoint:GetOrigin()
		self.endPointPosition = {} 
		self.endPointPosition[0] = GridNav:WorldToGridPosX(endPointVectorPosition.x)
		self.endPointPosition[1] = GridNav:WorldToGridPosY(endPointVectorPosition.y)
		
		-- Find each spawner location
		local spawners = Entities:FindAllByClassname( "npc_dota_scripted_spawner" )
		self.spawnerPositions = {}
		for _,v in pairs(spawners) do
			local spawnerVectorPosition = v:GetOrigin()
			local tempTable = {}
			tempTable[0] = GridNav:WorldToGridPosX(spawnerVectorPosition.x)
			tempTable[1] = GridNav:WorldToGridPosY(spawnerVectorPosition.y)
			table.insert(self.spawnerPositions, tempTable)
		end
		
		-- Find mainland marker
		local mainlandRefPointEntitiy = Entities:FindByName(nil, "mainland_ref_point")
		local mainlandRefPointVectorPosition = mainlandRefPointEntitiy:GetOrigin()
		self.mainlandRefPointPosition = {}
		self.mainlandRefPointPosition[0] = GridNav:WorldToGridPosX(mainlandRefPointVectorPosition.x)
		self.mainlandRefPointPosition[1] = GridNav:WorldToGridPosY(mainlandRefPointVectorPosition.y)
		
		-- Find island marker
		local islandRefPoint1Entitiy = Entities:FindByName(nil, "island_ref_point1")
		local islandRefPoint1VectorPosition = islandRefPoint1Entitiy:GetOrigin()
		self.islandRefPoint1Position = {}
		self.islandRefPoint1Position[0] = GridNav:WorldToGridPosX(islandRefPoint1VectorPosition.x)
		self.islandRefPoint1Position[1] = GridNav:WorldToGridPosY(islandRefPoint1VectorPosition.y)
    end
end
...
function BuildingHelper:InitGNV()
            ...
            if not terrainBlocked then
                ...
                for _,ent in pairs(blocked_map_zones) do
                    ...
                    if triggerBlocked then
						table.insert(self.bh_blocked_positions, position)
                        terrainBlocked = true
                        break
                    end
                end
    ...
    BuildingHelper.Grid = Grid:new(BuildingHelper.Terrain)
...
function BuildingHelper:SetCallbacks(keys)
    ...
    function keys:OnConstructionWouldBlockPath(callback) -- Called when player right clicks to cancel a queue
        callbacks.onConstructionWouldBlockPath = callback
    end
...
--[[	
	-- Check to see if adding building would block spawner paths
	if BuildingHelper:PathWillBeBlockedWithBuilding(construction_size, location) then
		-- Remove the model particle and Advance Queue
		BuildingHelper:AdvanceQueue(builder)
		BuildingHelper:ClearWorkParticles(work)
		...
		-- Building canceled, refund resources
		work.refund = true
		callbacks.onConstructionWouldBlockPath(work)
		return
	end
]]	
    ...
    if not option then
        for y = lowerBoundY, upperBoundY do
            for x = lowerBoundX, upperBoundX do
                BuildingHelper.Grid:set(x, y, BuildingHelper.GridTypes[grid_type])
            end
        end
        ...
        for y = lowerBoundY, upperBoundY do
            for x = lowerBoundX, upperBoundX do
                ...
                if not hasGridType then
                    BuildingHelper.Grid:set(x, y, BuildingHelper.Grid:get(x, y) + BuildingHelper.GridTypes[grid_type])
                end
            end
        end
        ...
        for y = lowerBoundY, upperBoundY do
            for x = lowerBoundX, upperBoundX do
                ...
                if not hasGridType then
                    ...
                    local distance = (current_pos - location):Length2D()
                    if distance <= radius then
                        BuildingHelper.Grid:set(x, y, BuildingHelper.Grid[y][x] + BuildingHelper.GridTypes[grid_type])
                    end
                end
            end
        end
...
function BuildingHelper:CellHasGridType(x, y, grid_type)
    if BuildingHelper.GridTypes[grid_type] then
        return bit.band(BuildingHelper.Grid:get(x, y), BuildingHelper.GridTypes[grid_type]) ~= 0
    end
end
...
function BuildingHelper:PathWillBeBlockedWithBuilding(size, location)
    local originX = GridNav:WorldToGridPosX(location.x)
    local originY = GridNav:WorldToGridPosY(location.y)
    local halfSize = math.floor(size/2)
    local boundX1 = originX + halfSize
	...
	BuildingHelper.Grid:save()

	-- Remove the bh_blocked tiles from the GridNav
	for i = 1, #self.bh_blocked_positions do
		local pos = self.bh_blocked_positions[i]
		BuildingHelper.Grid:set(GridNav:WorldToGridPosX(pos.x), GridNav:WorldToGridPosY(pos.y), BuildingHelper.GridTypes["BUILDABLE"])
	end
	

    -- Check all sqaures around proposed tower and store data in memory
    perimeterArray = {}

    for x = lowerBoundX - 1, upperBoundX + 1 do
        table.insert(perimeterArray, BuildingHelper.Grid:get(x, upperBoundY + 1))
    end

    for y = upperBoundY, lowerBoundY - 1, -1 do
        table.insert(perimeterArray, BuildingHelper.Grid:get(upperBoundX + 1, y))
    end

    for x = upperBoundX, lowerBoundX - 1, -1 do
        table.insert(perimeterArray, BuildingHelper.Grid:get(x, lowerBoundY - 1))
    end

    for y = lowerBoundY, upperBoundY do
        table.insert(perimeterArray, BuildingHelper.Grid:get(lowerBoundX - 1, y))
    end


    diffCount = 0
    for i = 1, #perimeterArray do
        if i ~= #perimeterArray then
            if perimeterArray[i] ~= perimeterArray[i+1] then
                diffCount = diffCount + 1
            end
        else
            if perimeterArray[i] ~= perimeterArray[1] then
                diffCount = diffCount + 1
            end
        end
    end
    ...
    if diffCount < 3 then
        BuildingHelper.Grid:undo()
        return false
    end

    -- Place building in proposed grid
    for y = lowerBoundY, upperBoundY do
        for x = lowerBoundX, upperBoundX do
            BuildingHelper.Grid:set(x,y,BuildingHelper.GridTypes["BLOCKED"])
        end
    end
	...
	local bottomLeftMinimapBoundary = Entities:FindByName(nil, "bottom_left_minimap_boundary")
	local bottomLeftMinimapBoundaryVector = bottomLeftMinimapBoundary:GetOrigin()
	local topRightMinimapBoundary = Entities:FindByName(nil, "top_right_minimap_boundary")
	local topRightMinimapBoundaryVector = topRightMinimapBoundary:GetOrigin()
	
	local y1 = GridNav:WorldToGridPosY(bottomLeftMinimapBoundaryVector.y) -1 
	local y2 = GridNav:WorldToGridPosY(topRightMinimapBoundaryVector.y) + 1
	for xx = GridNav:WorldToGridPosX(bottomLeftMinimapBoundaryVector.x) - 1 , GridNav:WorldToGridPosX(topRightMinimapBoundaryVector.x) + 1 do
		BuildingHelper.Grid:set(xx,y1,BuildingHelper.GridTypes["BUILDABLE"])
		BuildingHelper.Grid:set(xx,y2,BuildingHelper.GridTypes["BUILDABLE"])
	end
	
	local x1 = GridNav:WorldToGridPosX(bottomLeftMinimapBoundaryVector.x) - 1
	local x2 = GridNav:WorldToGridPosX(topRightMinimapBoundaryVector.x) + 1
	for yy = GridNav:WorldToGridPosX(bottomLeftMinimapBoundaryVector.y) - 1, GridNav:WorldToGridPosX(topRightMinimapBoundaryVector.y) + 1 do
		BuildingHelper.Grid:set(x1,yy,BuildingHelper.GridTypes["BUILDABLE"])
		BuildingHelper.Grid:set(x2,yy,BuildingHelper.GridTypes["BUILDABLE"])
	end
	
	-- Flood the end point
	local fillValue = 4
	flood4stack(self.endPointPosition[0],self.endPointPosition[1], BuildingHelper.Grid, Stack:new(), BuildingHelper.GridTypes["BUILDABLE"], fillValue)
	
	-- Check that each spawner would be able to get to the end point
	for k,v in pairs (self.spawnerPositions) do
		
		-- Calculates the path, and its length
		local mapValue = BuildingHelper.Grid:get(v[0], v[1])
		if not (mapValue == fillValue) then
            BuildingHelper.Grid:undo()
			return true
		end
	end
	
	-- Flood the island point
	fillValue = 8
	flood8stack(self.islandRefPoint1Position[0],self.islandRefPoint1Position[1], BuildingHelper.Grid, Stack:new(), BuildingHelper.GridTypes["BLOCKED"], fillValue)
	mapValue = BuildingHelper.Grid:get(self.mainlandRefPointPosition[0], self.mainlandRefPointPosition[1])
	if (mapValue == fillValue) then
        BuildingHelper.Grid:undo()
		return true
	end

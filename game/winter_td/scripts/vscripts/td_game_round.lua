...
function CTDlGameRound:OnEntityKilled( event )
	...
	if attackerUnit then
		...
		if playerStats then
			...
			if EntIndexToHScript( event.entindex_killed ):GetPlayerOwnerID() ~= playerID then
				playerKills[playerID] = playerKills[playerID] and playerKills[playerID] + 1 or 1

				CustomNetTables:SetTableValue( "kills", "_dynamic_player_" .. playerID, { kills = playerKills[playerID] } )
			end
		end
	end
end
